...
def f_OUT(w, A, R, time, dt, C, R2, f):
    ...
    oryginal_transmitancji = np.array(-oryginal_transmitancji)
    line4 = axs[1, 1].plot(t, np.convolve(f_IN3(A, w, time, dt), oryginal_transmitancji, mode='same'))
    ...
    if f == 1:
        ...
        print('2')
        f_IN2(A, w, time, dt)
        line4 = axs[1, 1].plot(np.convolve(f_IN2(A, w, time, dt), oryginal_transmitancji))
        print('22')
    else:
        print('3')
        f_IN3(A, w, time, dt)
